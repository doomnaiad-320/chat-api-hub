...
function renderIsStream(bool) {
  return bool ? (
    ...
    <Label
      style={{ color: grey[500], borderColor: grey[500] }}
      size='small'
      variant='outlined'
    >
      非流
...
export default function LogTableRow({ item }) {
  ...
  return (
    <TableRow tabIndex={item.id}>
      ...
      <TableCell
        onClick={() => handleCopyToClipboard(item.token_name)}
      ...
      <TableCell onClick={() => handleCopyToClipboard(item.model_name)}>
        <Label
          style={{ color: getModelColor(item.model_name) }}
          variant='outlined'
        >
      ...
      <TableCell>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <InputOutlined
            fontSize='small'
            sx={{ color: '#2196F3', marginRight: '4px' }}
          />
          {item.prompt_tokens || ''}
        </div>
      </TableCell>
      ...
      <TableCell>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <PaidOutlined
            fontSize='small'
            sx={{ color: '#FF9800', marginRight: '4px' }}
          />
              ...
              >
                {(() => {
                  try {
                    // 检查是否为图像相关模型
                    const isImageModel =
                      item.model_name && item.model_name.includes('gpt-image');
                    ...
                    return [
                      ...
                      multiplierObj.audio_ratio
                        ? `音频倍率: ${multiplierObj.audio_ratio}`
                        : null,
                      multiplierObj.audio_completion_ratio
                        ? `音频补全倍率: ${multiplierObj.audio_completion_ratio}`
                        : null,
                      '------------------------',
                      multiplierObj.text_input
                        ? `文本输入消耗: ${multiplierObj.text_input}`
                        : null,
                      multiplierObj.text_output
                        ? `文本输出消耗: ${multiplierObj.text_output}`
                        : null,
                      multiplierObj.audio_input
                        ? `音频输入消耗: ${multiplierObj.audio_input}`
                        : null,
                      multiplierObj.audio_output
                        ? `音频输出消耗: ${multiplierObj.audio_output}`
                        : null,
            ...
            >
              {(() => {
                try {
                  ...
                  return typeof item.multiplier === 'string' &&
                    item.multiplier.length > 30
